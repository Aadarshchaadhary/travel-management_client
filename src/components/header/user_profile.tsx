...
const UserProfile: React.FC<IProps> = ({ user }) => {
    ...
    if (isPending) {
         return <div className='absolute inset-0 backdrop-blur-2xl flex justify-center items-center'>
  ...
  return (
      <div className='flex items-center gap-2 min-w-[200px]'  >
          ...
          <div>
              <div>
                  <p className='text-lg font-semibold text-blue-600'>{user.first_name + ' ' + user.last_name }</p>
              </div>
              <div>
                  <p onClick={()=>{mutate()}} className='text-[16px] text-red-500 cursor-pointer'>Logout</p>
              </div>
          </div>
    </div>
  )
